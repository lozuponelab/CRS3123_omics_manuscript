...
#SBATCH --output=/scratch/alpine/you@xsede.org/webin/crestone/slurm/%J-%x-%a.out
#SBATCH --error=/scratch/alpine/you@xsede.org/webin/crestone/slurm/%J-%x-%a.err
...
samp_ID=$(sed -n "${index_num}p" /projects/you@xsede.org/webin_cli_submission/ID.txt)

echo "submitting ${samp_ID}...."

#The samples are submitted, parameters need to be adjusted to your login information
java -jar /projects/you@xsede.org/software/webin-cli-9.0.3.jar \
    -context=reads \
    -manifest="/projects/you@xsede.org/webin_cli_submission/crestone_ebi_perSample_manifests/${samp_ID}_manifest.txt" \
